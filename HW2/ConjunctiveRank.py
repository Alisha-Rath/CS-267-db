...
def preProcessQueries(input_queries):
    ...
    for i in range(len(queries)):
        sublist = queries[i]  # Access the sublist at index i

        # Iterate through each element in the sublist
        for j in range(len(sublist)):
            # Update the element at index j in the sublist
            translator = str.maketrans("", "", string.punctuation + string.digits)
            data_without_punctuation = queries[i][j].translate(translator)
            queries[i][j] = data_without_punctuation.lower()
...
def main():
    ...
    if len(sys.argv) < 4:
        print("Invalid Usage")
        print(
            "Example Usage: python Conjunctive.py filename.txt num_of_results queries"
        )
        return
    ...
    rankCosineScores = rankCosine(queries, num_of_results)

    print("DocId Score")
    for rankCosineScore in rankCosineScores:
        print(rankCosineScore[0], " ", rankCosineScore[1])
...
def binary_search_next(P, low, high, pos):
    print("Perform a binary search on a posting list", P)
...
def next(term, docid, pos):
    print("Perform a next ADT using gallop search to find the next position of a term in a document ", docid, " after a given position: ", pos)
...
def prev(term, docid, pos):
    print("Perform a prev ADT using gallop search to find the prev position of a term in a document ", docid, " before a given position: ", pos)
...
def nextPhrase(terms, pos):
    print("Perform a nextPhrase ADT to find the next docId:position of terms", terms,  " in a document after a given position.", pos)
    global inverted_index
    v = pos
    ...
    if v[1] - u[1] == len(terms) - 1:
        print("Found the phrase", terms, "here ", u, v)
        return u
    else:
...
def prevPhrase(terms, pos):
    print("Perform a prevPhrase ADT to find the prev docId:position of terms", terms,  " in a document before a given position.", pos)
    """
    ...
    if (u[1] - v[1]) == len(terms) - 1:
        print("Found the phrase", terms, "here ", v, u)
        return v
    else:
...
def get_doc_id(u):
    print("Docid for u", u[0])
    return u[0]


def docRight(termsList, currentDoc):
    print("Perform a docRight ADT to find the next docId where query ", termsList,  "is present after a given docid: ", currentDoc)
    ...
    for i in range(currentDoc, maxDocuments + 1):
        ...
        if found:
            doc_id = get_doc_id(u)
            print("Found all terms in query in docid: ", doc_id)
            return doc_id
...
def docLeft(termsList, currentDoc):
    print("Perform a docLeft ADT to find the prev docId where query ", termsList,  "is present before a given docid: ", currentDoc)
...
def freqTermInQuery(term, terms):
    ...
    for t in terms:
        if term == t:
            count += 1
    print("Freq of:", term, " in query is :", count)
    return count
...
def tfidf(freqTerm, totDocWithTerms):
    ...
    if idf == 0:
        print("query present in all docs hence idf is 0 but we are setting it to 1 to just consider the tf score and avoid division by zero in cosine similarity process")
        idf=1
    return tf * idf
...
def createVectorQuery(terms):
    ...
    for term in terms:
        ...
        score = tfidf(qt, Nt)
        print("Tfidf score for: ", term, " in query is: ", score)
...
def createVectorDoc(terms, doc_id):
    ...
    for term in terms:
        ...
        Nt = len(inverted_index.get(term, {}))  # Corrected Nt calculation
        print("term is", term, "...posting list is", p, "...len of posting list is", ft, "...Nt is", Nt)
        score = tfidf(ft, Nt)
        print("Tfidf score for: ", term, " in doc_id: ", doc_id,   " is: ", score)
...
def calculate_unit_vector(vector):
    print("vector to create unit vector", vector)
    ...
    if magnitude == 0:
        raise ValueError("The query is valid for all documents. Cannot calculate the unit vector for a zero vector.")
...
def sim(query, doc_id):
    ...
    print("Unit vector of docid: ", doc_id, " is: ", unit_vector_doc)
    print("Unit vector of query is: ", unit_vector_query)
    ...
    print("Cosine similarity of docid: ", doc_id, " and query is: ", similarity_score)
...
def rankCosine(queries, k):
    ...
    print("Document that contains all the query terms: ", doc_id)
